...
# Script para corrigir problemas do banco no VPS
# Execute este script no VPS para resolver os erros de configuração

echo "🔧 Corrigindo configurações do banco no VPS..."
...
# Executar correções SQL diretamente
...
if [ $? -eq 0 ]; then
    echo "✅ Correções SQL executadas com sucesso!"
else
    echo "❌ Erro ao executar correções SQL"
    exit 1
fi
...
echo "✅ Correção concluída! Tente iniciar a aplicação agora."